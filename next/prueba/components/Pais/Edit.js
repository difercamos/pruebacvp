...
export default class extends React.Component {
    ...
    componentWillMount = () =>{
        fetch('http://localhost/api/v1/countries/'+this.props.id, {
            method: 'GET',
            headers: {
            ...
            .then((res) => res.json())
            .then((country) => this.setState({country: country, name: country.name}));
    }
    ...
    async handleSubmit() {
        ...
        const res = await fetch('http://localhost/api/v1/countries/'+this.props.id, {
            method: 'PUT',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({"id": this.props.id, "name": this.state.name})
        });
